...
- **价格和户型已上架**：51 个小区中 28 个有价格数据， 列表卡片显示单间均价, 详情卡片显示价格表
- **CSV→JSON 同步**：`data/房源数据存档.csv` 为唯一数据源, 通过 `sync-csv.js` 同步到 `communities.json`
- 租金同步脚本：`node scripts/sync-csv.js`（`--dry-run` 预览差异）
